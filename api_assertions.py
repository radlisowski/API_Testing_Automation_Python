def assert_user_attributes(expected_user, actual_user):
    ...
    Parameters:
            ...
            - actual_user: An object containing the actual user data obtained from a source like a server
...
def assert_user_addresses(expected_user, actual_user):
    ...
    for index, (original_address, actual_address) in enumerate(zip(expected_user.addresses, actual_user.addresses)):
        ...
        assert len(original_address.phone_numbers) == len(
            actual_address.phone_numbers
        ), f"Number of phone numbers do not match at index {index}."

        for phone_index, (original_phone, actual_phone) in enumerate(
            zip(original_address.phone_numbers, actual_address.phone_numbers)
        ):
            assert original_phone.type == actual_phone.type, f"Phone types do not match at phone index {phone_index}."
            assert (
                original_phone.number == actual_phone.number
            ), f"Phone numbers do not match at phone index {phone_index}."
