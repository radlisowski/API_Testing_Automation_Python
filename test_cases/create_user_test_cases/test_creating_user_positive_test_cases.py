...
import constants
from api_assertions import assert_user_addresses, assert_user_attributes
...
def test_post_user_endpoint_with_valid_payload(get_user_db_collection):
    payload = User(
        ...
        email=f"test_{random.randint(1000000, 9999999)}@dummypage.com",
        role="tester",
        addresses=[
            Address(
    ...
    response = requests.post(get_url("user"), json=payload.model_dump(mode="json"))
